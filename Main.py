...
# Inicializar pygame antes de crear la pantalla
pygame.init()
...
class Juego():
    def __init__(self):
        ...
        # Ejemplo: una pared en el centro de la pantalla
        pared1 = Pared(300, 200, 100, 30)
        self.paredes.add(pared1)

        # Agrega las paredes al grupo general de sprites si deseas que se dibujen
        self.sprites.add(self.paredes)
    ...
    def update(self):
        ...
        self.cazador.update(self.mono)  # Cazador persigue al mono (asumiendo tienes este método en Cazador)

        if self.vida <= 0:
            escena = Gameover(self.puntos, self.nivel)
            return

        self.puntosnivel = self.puntos / self.nivel if self.nivel != 0 else 0
        ...
        # --- Colisión mono-cazador (quitar vida pero NO terminar el juego) ---
        if pygame.sprite.collide_rect(self.cazador, self.mono):
            self.mono.grito_perder()
            self.cazador.risa()
            self.vida -= 15
            self.mono.rect.x += 40  # Empuja al mono a la derecha para evitar daño instantáneo repetido
            if self.vida < 0:
                self.vida = 0
